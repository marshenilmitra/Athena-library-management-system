...
def init_db():
    ...
    if cursor.fetchone()['count'] == 0:
        authors = [('Robert C. Martin', 'Clean Code author'), ('Andrew Hunt', 'Pragmatic Programmer author'), ('Erich Gamma', 'Design Patterns author')]
        for name, bio in authors:
            cursor.execute("INSERT OR IGNORE INTO authors (name, bio) VALUES (?, ?)", (name, bio))
        
        publishers = [('Prentice Hall', 'USA'), ('Addison-Wesley', 'USA'), ('O\'Reilly Media', 'USA')]
        for name, addr in publishers:
            cursor.execute("INSERT OR IGNORE INTO publishers (name, address) VALUES (?, ?)", (name, addr))
        
        categories = [('Software Engineering', 'Books about software design and best practices'), ('Computer Science', 'Foundational CS topics'), ('Data Science', 'Machine Learning & Analytics')]
